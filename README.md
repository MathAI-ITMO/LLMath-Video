...
- Секреты и параметры LLM больше не хранятся в `config.json`. Создайте файл `.env` в корне проекта (рядом с `app.py`) и добавьте переменные:
  ...
  - `VIDEOAPP_URL_PREFIX` — путь, под которым приложение обслуживается (например `some-path` для `https://example.com/some-path`). Можно также задать в `config.json` в `server.url_prefix`.
...
### Запуск под путём (subpath)
Чтобы обслуживать приложение по адресу вида `https://example.com/some-path`, задайте префикс:
- **Переменная окружения:** `VIDEOAPP_URL_PREFIX=some-path` (без ведущего слэша).
- **Или в `config.json`:** в секции `server` добавьте `"url_prefix": "some-path"`.

Прокси (Nginx и т.п.) должен передавать полный путь на приложение (например `location /some-path/ { proxy_pass http://backend:5001/some-path/; }`).
