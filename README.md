...
### 3.2) Hosting under a subpath (e.g. https://example.com/some-path)
Чтобы раздавать приложение по пути на домене (например `https://example.com/llmath-video`), задайте базовый путь одним из способов:
- **config.json** — в секции `server` добавьте `"base_path": "/some-path"` (без завершающего слэша).
- **Переменная окружения** — `VIDEOAPP_BASE_PATH=/some-path` (переопределяет config).

После этого все маршруты и статика будут работать под этим префиксом; UI автоматически подставляет его в запросы к API.
